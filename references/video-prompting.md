...
🟢 **Mode routing comes first [H-off/X-ext]:** every job picks one of four modes BEFORE any prompt is written — **`t2v`** (new shot from prose + refs) · **`omni_reference`** (new shot with a source video attached as reference: performance re-skins, green-screen relocations; duration settable, source ≥4 s) · **`video_edit`** (scoped fix inside an approved take; duration + ratio locked to source ±0.3 s) · **`video_extension`** (forward/backward, 4–30 s per round). The same sentence means different things per mode; two modes silently lock parameters.
