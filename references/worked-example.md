...
| Lint | words 601 · 3,375 characters of ~5,000 · negations 10 (`not frame 1`, `not before`, three `Do not copy` clauses, `no drift mid-segment`, `no lens flare`, `no music, no BGM`, `no grain` — B's habitual surplus; the economy doctrine keeps the AUDIO exclusion and writes the others only after the fault has occurred) · numerals outside TC: FOV, Kelvin, percent — deliberate look controls of technique B, listed in Crew choices · absolute measures 2 (the camera distances in metres — a measure without a referent even in B: write them as relations, SKILL rule 7) · double mentions: the lever state, the hatch–lever axis and the wet fur appear in ACTION/LOCATION MAP and again in POSITIVE LOCKS — a lock lives once (ch. 12 fence) |
