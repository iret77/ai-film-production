...
This file preserves **model prompt profiles and historical production evidence**, not current platform UI/MCP selectors. Every production use must append `platform → surface → model + mode → version/snapshot → source + checked date` as required by `SKILL.md`.

- **Current Higgsfield UI/MCP authority:** [`platform-ui-workflows.md`](platform-ui-workflows.md), notably `HF-CS4@2026-08-31`, `HF-WEB@2026-08-31`, and `HF-MCP@2026-08-01`.
...
## 13. Higgsfield Cinema Studio — archival 3.5 profile and historical notes
...
> **Version scope:** `HF-CS3.5-HIST / Cinema Studio 3.5 / source date unknown`; selected notes labelled “4.0” below are `HF-CS4-HIST@2026-08` and are superseded for live UI routing by `HF-CS4@2026-08-31` in `platform-ui-workflows.md`.
...
**Do not copy settings, limits, or model availability from this subsection into a current recommendation.** It remains only as dated production evidence; the current verified UI/MCP record is `platform-ui-workflows.md` → `HF-CS4@2026-08-31` / `HF-WEB@2026-08-31` / `HF-MCP@2026-08-01`.
...
🟢 **Seedance 2.5 edit/extension surface on Higgsfield (Aug 2026) [H-off/X-ext]:** four modes behind one model — `t2v` · `omni_reference` · `video_edit` · `video_extension` — plus UI features Region edit, Shot re-generate, Draw to Video, Extend/Transition/Reverse. 🟢 **In the composer UI, `video_edit` surfaces as its own model entry "Seedance 2.5 Edit" [PP]:** video-model selector → "Seedance 2.5 Edit", task dropdown "Edit video", source clip into the **VIDEO TO EDIT** slot, references addressed via @-elements as usual, up to 1080p — sessions should name it exactly like this when guiding the user. **No mask/annotation editor** (that's Dreamina-only) — edit regions are described in words. **Edits bill the source's full duration** (a 20-s master costs a 20-s render however small the edit) — savings come from fewer attempts, not cheaper runs. Native **1080p since 2026-08-14** (Plus+ plans); 4K on 2.5 is ALWAYS an upscale (FLUX 3 Video Upscaler keeps audio in sync) — native 4K stays the Seedance 2.0 lane. **Soul ID** carries a trained identity across ALL platform models ("generate with Seedance 2.5, switch models, same character holds"); AI Cast / Element Sharing / Canvas make the reference pool team-shared. Workflow doctrine: video-prompting ch. 14b.
