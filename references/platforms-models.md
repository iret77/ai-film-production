...
🟢 **Named elements include anchors [PP]:** create characters, locations, props AND approved shot anchors as named elements (production-pipeline ch. 1, `@anchor_1A` convention); the @tag brief then addresses the anchor directly ("start from @anchor_1A") — no manual upload per generation, no wrong-image mistakes as the anchor library grows.
