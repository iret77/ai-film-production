...
🟢 Refines the production-pipeline ch. 8 hard rule: there IS a reliable prompt-only path to flat stylized environment stills with no character in frame — the style must be actively FORCED and every realism path explicitly blocked. Five building blocks that work only together:
