...
🟡 [our read of the published frames] **Character sheet as a CG turnaround:** A-pose front and back full-body plus a large face close-up, neutral cream background, rendered as a finished 3D character (fabric weave on the cap, glasses, moustache fibres). The sheet defines the project's stylization level (ch. 8) — here "finished CG character" — while the world stays painted. Small props on the sheet (thin glasses) are identity here, not clutter; the accessory rule (production-pipeline ch. 3) applies to non-identity details only.
