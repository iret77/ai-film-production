...
**Research snapshot:** 31 August 2026 · **re-verified against the primary sources:** 4 September 2026 (all `*@2026-09-04` keys below; only Arcads web keeps the 2026-08-31 snapshot; `FILMFLOW-WEB@2026-09-07` was added and live-checked on 7 September 2026; the OpenAI image keys were refreshed to the **GPT Image 2.5** family on 9 September 2026 — `CGPT-IMG@gpt-image-2.5@2026-09-09`, `OPENAI-IMG-API@gpt-image-2.5@2026-09-09`, API model list live-read that day)  
...
| `CGPT-IMG@gpt-image-2.5@2026-09-09`; `OPENAI-IMG-API@gpt-image-2.5@2026-09-09` | **Supersedes the former `…@gpt-image-2@2026-09-04` keys.** OpenAI announced **ChatGPT Images 2.5** on 2026-09-08 (announcement read first-party 2026-09-09) 🟢 [OAI-off]. ChatGPT web/app is an unversioned chat surface now running "ChatGPT Images 2.5", rolling out to **all** ChatGPT, ChatGPT Work and Codex tiers on desktop/mobile/web; new surface features: **Sketch** (draw in chat via `@Sketch` as a visual guide), **Templates** (popular formats — Poster, Merch, flyers, product photos), on-image **comments** for focused edits, and shareable prompts [OAI-off] — recheck the live app for exact controls. Outputs carry **C2PA metadata + invisible watermarking** ([system card](https://deploymentsafety.openai.com/chatgpt-images-2-5)) → the disclosure/archive discipline of post-audio-legal ch. 20 applies. The API is a separate surface with **two** models: `gpt-image-2.5-flare` — the **default**, "higher-quality images than GPT‑Image‑2 at 50% lower latency", for everyday/high-volume, social/creator, product, visual-search and prototyping work; and `gpt-image-2.5-sunburst` — "an extra level of precision for detailed creative work with longer generation times", for production-ready campaign creative and polished product imagery. Both bring the quality/editing/speed improvements (Flare is not the "no-edit" tier). The guide **no longer lists** `gpt-image-2` / `gpt-image-1.5` / `gpt-image-1` / `gpt-image-1-mini` (superseded; API guide live-read 2026-09-09) 🟢 [OAI-off]. Documented API params: sizes `1024x1024` / `1536x1024` / `1024x1536` (custom `WIDTHxHEIGHT`, multiples of 16, aspect 1:3–3:1, neither edge > 3840 px); quality `low|medium|high|xhigh|max|auto`; background transparent\|opaque\|auto; format `png|jpeg|webp` 🟢 [OAI-off]. Responses image-generation tool unchanged. **Vendor improvement claims over Images 2.0 — [OAI-off] first-party, but 🔴 (verify in a production A/B before relying):** ~50% lower latency; better subject/identity preservation from reference photos; better **edit locality** (edit only what's asked, keep the rest — even complex subjects/backgrounds) and multi-turn edit consistency without quality decay; more accurate real-world/text content; more complex layouts + transparent backgrounds; more consistent style hold with less drift as instructions get specific. 🟡 Named early customers: Higgsfield AI, Adobe, Manus, Runway — Higgsfield's Head of Product praises Flare's "understands what not to change… across film, UGC and advertising" (vendor testimonial, not independent evidence). | [ChatGPT image-generation guide](https://learn.chatgpt.com/docs/image-generation) · [OpenAI API guide](https://developers.openai.com/api/docs/guides/image-generation) · [API pricing](https://developers.openai.com/api/docs/pricing#image-generation) · [Images 2.5 announcement](https://openai.com/index/introducing-chatgpt-images-2-5/) · [system card](https://deploymentsafety.openai.com/chatgpt-images-2-5). API model list + params live-read 2026-09-09; announcement read first-party (user-provided page text) 2026-09-09. **Re-verify the improvement claims in production before treating them as 🟢** — especially edit locality (image-model-logic 24g). |
...
| **ChatGPT** | A conversational image-generation surface running **ChatGPT Images 2.5** (API `gpt-image-2.5-flare` / `gpt-image-2.5-sunburst`), plus an optional host for connected third-party MCP apps. | Use it for conversational still creation/editing and reference-guided iteration. ChatGPT-native image generation is not a video generator and does not inherit an MCP provider’s web UI. |
...
3. **ChatGPT native:** ChatGPT chat → built-in ChatGPT Images 2.5 (`gpt-image-2.5` family).
...
Treat **ChatGPT itself** as a first-class *still-image* route. In a normal ChatGPT chat, the user can describe an image, attach reference image(s), then request iterative or targeted edits in the same conversation. The current built-in surface is **ChatGPT Images 2.5** (announced 2026-09-08; it supersedes GPT Image 2 — the API models are `gpt-image-2.5-flare` for fast/high-volume work and `gpt-image-2.5-sunburst` for edit-precision/production work). This is appropriate for concept frames, plates, product/still iteration and image edits before handing an approved result to a video platform. It is **not** a native replacement for Seedance, Cinema Studio, an Arcads actor, or any other video/film workspace. [ChatGPT image-generation guide](https://learn.chatgpt.com/docs/image-generation)
...
| Multiply a working ad into variants (new product, new character, same structure) | **Marketing Studio / Ad multiplier** through the Higgsfield MCP in Claude ("multiply my top performing ad", [P54] 2026-09-08; live tool `get_workflow_instructions({workflow:"ad-multiplier"})`, `HF-MCP@2026-09-12`) | A workflow over an uploaded ad used as template, not a generator — approve the brief and every output (genre-baselines Commercial, disclosure duties ch. 20). |
| Motion-design deliverables (SaaS launch video, motion graphics over a talking head, hypermotion product ad, 3D real-estate flythrough, 2D and editorial explainers) | **Marketing Studio** presets — via its web page (Marketing Studio → Video) or through the Higgsfield MCP in Claude, which picks the preset from the brief [H-off, P68, 2026-09-02] | Vendor workflow: client URL + logo in the chat → the studio's agents analyse product and audience → preset chosen → video; a finished motion video dropped in as reference is recreated "one-to-one". Vendor tips: generate a NINE-FRAME storyboard first and fix layout at the image stage before rendering; for motion graphics over real footage ask for the design sheet in HTML ("fonts arrive better"), pick the brand font, approve, then generate. Cost snapshot 2026-09: ≈1,800 credits (~$70) for six style samples including failed takes. Text in these outputs is renderability §2 exception (c) — check every word and number. |
...
| `video_edit` | model entry "Seedance 2.5 Edit", task "Edit video", VIDEO TO EDIT slot; [P55, 2026-09-06] also the practitioner route for RE-SKINNING phone footage (raw clip + character/wardrobe/prop/set references — the performance-capture path, production-pipeline ch. 9) | `HF-WEB@2026-09-04` [PP] + P55 |
| `video_extension` | the Extend control (product page: "Extend/Transition/Reverse"; Cinema Studio 4.0: "Forward/Backward Extend"); 🟡 [P17, 2026-08-23] the extend form asked for a direction as **sequel / prequel** (= forward / backward) with a settable duration (a 29-s sequel ran) — read the live label | `HF-CS4@2026-09-04` + P17 |
...
🟡 Credit orders of magnitude on OpenArt, Aug 2026 [P56] — dated, read the live estimate: Seedance 2.0 ≈3,000 credits per 15 s at 1080p, ≈6,000 at 4K · Seedance 2.5 ≈1,955 per 15 s and ≈3,905 per 30 s at 720p (1080p arrived later at a higher price) · MiniMax H3 ≈1,350 per 15 s at 2K · Flux 3 Video ≈4,000 per 20 s at 1080p. The same test dates the picker (Flux 3 Video, MiniMax H3 and both Seedance versions selectable in Aug 2026).
...
| Create or precisely revise a still in a conversational workspace | **ChatGPT native — ChatGPT Images 2.5 (`gpt-image-2.5` family)** | It supports prompt-led reference use and iterative follow-up edits in one chat. | A native video render, external provider workspace, or API-only output controls. |
