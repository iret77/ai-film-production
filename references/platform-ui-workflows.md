...
| `HF-BRIDGE-MCP@2026-09-02` | Higgsfield Blender **Bridge** MCP at `https://bridge.higgsfield.ai/mcp`, paired with the Blender add-on (higgsfield.ai/plugins/blender); unversioned, add-on page checked 2026-09-02 [P50]. Tool/schema list NOT documented publicly — read it in the connected session (MCP execution gate step 2). | [Add-on page](https://higgsfield.ai/plugins/blender) · sources.md P50. Recheck exposed tools, credit price on Generate and plan-level exclusions before use. |
...
> **Version scope:** `HF-CS4@2026-08-31` applies only to Cinema Studio 4.0 and its project-specific claims; `HF-WEB@2026-08-31` applies to the other Higgsfield web tools. `HF-MCP@2026-08-01` applies only when explicitly named. Cinema-specific controls never inherit into standalone Video/Image/Audio/Edit tools. `HF-BRIDGE-MCP@2026-09-02` applies only to the Blender add-on's Bridge connector and never to `mcp.higgsfield.ai/mcp`.
...
|---|---|---|
| `t2v` | model entry "Seedance 2.5", no source clip attached — task label unverified, read the live task dropdown | `HF-WEB@2026-08-31` |
| `omni_reference` | model entry "Seedance 2.5" with the source clip attached as a REFERENCE (Higgsfield's marketing name for this is "R2V motion guidance") — NOT the "Seedance 2.5 Edit" entry, which locks duration/ratio and bills the source length (production-pipeline ch. 8). Exact task label unverified, read the live task dropdown | `HF-WEB@2026-08-31` + P21 |
| `video_edit` | model entry "Seedance 2.5 Edit", task "Edit video", VIDEO TO EDIT slot | `HF-WEB@2026-08-31` [PP] |
| `video_extension` | the Extend control (product page: "Extend/Transition/Reverse"; Cinema Studio 4.0: "Forward/Backward Extend") | `HF-CS4@2026-08-31` |
