...
**Short form in agent output:** '`KEY` — [surface] / [model + mode] / [access route] · live-checked YYYY-MM-DD' or '… · from reference `KEY` (checked <its date>), not live-checked this session.' The stored form is the RECEIPT line. Never write the live-checked form for a check you did not perform. If a user sees a different label/dropdown/tool list, the live UI supersedes this reference and the agent mints a new project KEY with today's check date per the SKILL.md KEY grammar and records its RECEIPT line in bible section B1b.
...
7. **Receipt state.** Write it in one of the two states of SKILL rule 17 (`live-checked <date>` | `from reference <KEY>`); the ledger above is the 'newest stored receipt' when no bible B1b row exists.
...
4. **OpenAI API:** the RECEIPT chain with access route = API.
...
MCP, plugins and Canvas always bill standard credits; web-only Unlimited does not apply.
