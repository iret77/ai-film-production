...
- **Prompt shot list** (when the user wants generation-ready prompts): one connected document, not loose prompts — a **style prefix** at the top (light, camera, color; glued to every prompt, per-scene overrides allowed) and **named prompts** (1A, 1B, 2A …) so single prompts can be edited in isolation. Write choreography move by move (never "he dances"), bind opening cuts to the previous scene's closing motion for match cuts, and flag any shot whose geography needs a **layout map** (GPT Image schematic as prompt input) instead of prose. Every prompt that uses references MUST contain a reference block (one line per reference: tag + "controls only [job]" + "do not copy [exclusions]") and MUST end with an attachment checklist in upload order — a reference the prompt does not address does not exist for the model. For stylized looks (Pixar, cartoon, anime), apply the full style stack from style-control.md; a style word alone will be ignored.
