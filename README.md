...
v3.1.2-en (2026-09). Universal — contains nothing project- or person-specific. This repository is the source of truth for the skill; lessons from production flow back here via commits, and every release ships an upload-ready `.skill` package.

### Changelog

- **v3.1.2-en (2026-09-09)** — **GPT Image 2.5 integrated.** The OpenAI still-model ledger refreshed to the `gpt-image-2.5` family — `gpt-image-2.5-flare` (fast/everyday) and `gpt-image-2.5-sunburst` (edit-precision/production) — superseding `gpt-image-2`, with new API specs (native sizes to 3840 px/edge, quality/background/format enums), the ChatGPT-native "Images 2.5" surface (Sketch/Templates/on-image comments) and receipt keys `CGPT-IMG@gpt-image-2.5@2026-09-09` / `OPENAI-IMG-API@gpt-image-2.5@2026-09-09`. The vendor improvement claims (edit locality — image-model-logic 24g; identity/style hold; ~50% speed) are tagged 🔴 pending a production A/B; the `[PP]`/`[P]` findings established on the gpt-image-2 generation are preserved with provenance notes. Routing and token grammar updated to 2.5; no rule or structure changes. Third-party model pickers still listing "GPT Image 2" stay frozen at their receipt dates.
